...
class ControlTests {
    ...
    def testMultiCases(): Unit = {
        ...
        val cmdStr: String = "stop!"
        cmdStr.toLowerCase match {
            case "start" | "go" | "y" | "yes" => println("affirmative")
            case "stop" | "quit" | "n" | "no" => println("negative")
            case _ => println("unknown " + cmdStr.toLowerCase)
        }
        ...
        val cmd: Command = Stop
        cmd match {
            case Start | Go => println(Start)
            case Stop | Whoa => println(Whoa)
        }
    }
